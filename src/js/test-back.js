 ...
 * Exécuter avec : node --experimental-vm-modules src/js/test-back.js
...
async function runTests() {
  ...
  try {
    const artistes = await getArtistesByDate();
    const genre = artistes.find(a => a.genre_musical)?.genre_musical;
    if (genre) {
  ...
  try {
    const artistes = await getArtistesByDate();
    const premier = artistes.find(a => a.dates_de_representation);
    if (premier) {
